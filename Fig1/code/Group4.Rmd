...
利用したデータが1970年から2000年までは10年間隔、それ以降は5年間隔で行われたため、グラフにした際に欠損値があった。

Goldinはそれに対して前後の調査回コーホートの平均値を使用したため、それに即した結果は3.2で示してある。
...
## Goldin(2014)に合わせたグラフの作成


```{r}
df_long_f3 <- df_long_f |>
  complete(age, birth_year) |>
  arrange(age, desc(birth_year)) |>
  group_by(age) |>
  mutate(wage_gap2 = if_else(
    !is.na(wage_gap), wage_gap, (lag(wage_gap) + lead(wage_gap))/2
  )) |>
  ungroup() |>
  filter(!is.na(wage_gap2))
...
```{r}
# グラフ２
G2 <- ggplot(df_long_f3, aes(x = age, y = wage_gap2, color = factor(birth_year), group = birth_year)) +
   geom_line(size=1) +
  geom_point() +
  labs(
    x = "Age",
...
- 男女間の賃金の格差は20世紀後半にかけて全体的に縮小し、U字が平たくなった。
...
- 50代を超えると、子育てを終えた女性が職場復帰するため職業選択の幅が広がり、男女間の格差も縮小する。
